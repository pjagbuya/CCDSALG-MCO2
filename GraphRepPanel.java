...
public class GraphRepPanel extends JPanel
{   
    ...
    protected void paintComponent(Graphics g) 
    {
        ...
        for(Node temp : uniqueNodes)
        {
            ...
            if(uniqueNodes.indexOf(temp)==1)
                temp.setNum(392850);
            for(Node potRelNode : uniqueNodes)
            {
                j+= 1;
                if(temp.isRelated(potRelNode))
                {
        ...
        if(updateLine)
        {
            ...
            g.drawLine(XPOS.get(uniqueNodes.indexOf(selectedNode))  + 20 + ((selectedNode.getSelfNum()+"").length()-1)*10, startY-5, XPOS.get(uniqueNodes.indexOf(selectedNode)) + 20 + ((selectedNode.getSelfNum()+"").length()-1)*10, startY - 5  + uniqueNodes.size()*50+50);
            g.drawLine(XPOS.get(uniqueNodes.indexOf(selectedNode)) -15 + ((selectedNode.getSelfNum()+"").length()-1), startY-5, XPOS.get(uniqueNodes.indexOf(selectedNode)) -15 + ((selectedNode.getSelfNum()+"").length()-1), startY - 5  + uniqueNodes.size()*50+50);
        }
